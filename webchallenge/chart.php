...
<div id=main>
<div class=title>關注項目</div>
<table width=100% height=90% cellspacing=0　cellpadding=0 style="padding-top:5px;padding-left:5px;">
<?php
$pdo=con();
$pdo->query('SET NAMES `utf8`');
$dbh=$pdo->prepare("SELECT * FROM `data` ORDER BY `search` desc");
$dbh->execute();
$max=$row=$dbh->fetch();
$res=400;
echo "<tr onclick=\"javascript:parent.location.href='data.php?keyword={$row['topic']}'\" class=item>
