...
$dbh=$pdo->prepare("SELECT * FROM `chat` ORDER BY `time` desc LIMIT 0,10");
$dbh->execute();
while($row=$dbh->fetch())
{
echo "<div class=commentmain><div class=commentbox>{$row['content']}<br></div><div class=commentconner></div><br>
<a href=\"data.php?keyword={$row['topic']}\"><div class=commentuser style=\"display:inline; color:rgb(34, 193, 255)\">{$row['commenter']}</div>
